...
class RouteQualityEvalTest {
  ...
  @Test
  void freeFormRoutesSkipQualitySuite() {
    RouteRow shopassist = routes.activeRoute("shopassist_case").orElseThrow();
    assertThat(shopassist.evalSuiteId()).isNullOrEmpty();
    assertThat(shopassist.workflowId()).isNullOrEmpty();

    RouteRow overdraft = routes.activeRoute("overdraft_fee_qa").orElseThrow();
    assertThat(overdraft.evalSuiteId()).isNullOrEmpty();
    assertThat(overdraft.workflowId()).isNullOrEmpty();
  }
  ...
  void reorderedWorkflowFailsMatch() {
    RouteQualityCase evalCase =
        RouteQualityJson.loadActive("duplicate_charge_review_tools").cases().getFirst();
    Workflow workflow = workflows.find("duplicate_charge_review", "2026.08.1").orElseThrow();
