...
class JobsEntitleEvalTest {
  ...
  void setUp() {
    decide =
        new DecideService(
            new CatalogueService(new InMemoryRouteStore().seedEvalBoard()),
  ...
  @Test
  void hiddenKycEntitlesWhenClaimed() {
    EvalCase entitled =
        FILE.cases().stream()
            .filter(c -> "jobs-kyc_onboarding-entitled".equals(c.id()))
            .findFirst()
            .orElseThrow();
    DecideResult result =
        decide.decide(
            new DecideRequest(
                "jobs", "api", "eval-hidden", null, "kyc_onboarding", entitled.claims()),
            "afd");
    assertThat(result.outcome()).isEqualTo("route");
    assertThat(result.routeId()).isEqualTo("kyc_onboarding");
  }
}
