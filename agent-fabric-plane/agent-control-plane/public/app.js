...
const AUTONOMY_MODE_SHORT_LABEL = {
  0: "Single",
  1: "Autonomous",
  2: "Deterministic",
  3: "Guided",
...
function countAutonomyModes(routes) {
  const counts = { 0: 0, 1: 0, 2: 0, 3: 0, total: 0 };
  for (const route of routes ?? []) {
    const mode = Number(route?.autonomy_mode);
...
function autonomyModeSummary(counts) {
  return [0, 1, 2, 3]
    .map((mode) => `${AUTONOMY_MODE_SHORT_LABEL[mode]} ${counts[mode] ?? 0}`)
    .join(" · ");
}
...
function buildCatalogRow({ title, href, icon, counts, summary = null }) {
  ...
  link.href = href;
  const body = el("span", "catalog-row__body");
  body.append(el("span", "catalog-row__name", title));
  if (summary) {
    body.append(el("span", "catalog-row__summary", summary));
  }
  link.append(catalogIcon(icon), body);
...
async function showLanding() {
  ...
  const routeAutonomySummary = autonomyModeSummary(
    countAutonomyModes(routes.filter((route) => route.active === true)),
  );
