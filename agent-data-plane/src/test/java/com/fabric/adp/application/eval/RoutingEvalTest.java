...
class RoutingEvalTest {
  ...
  @BeforeEach
  void setUp() {
    catalogue = new CatalogueService(new InMemoryRouteStore().seedDemo());
  ...
  void chatRouting(String id, EvalCase evalCase) {
    ...
    if ("route".equals(evalCase.expected().outcome())) {
      ...
      assertThat(result.routeId())
          .isNotIn("card_freeze", "account_notify", "kyc_onboarding", "purchase_refund", "fraud_investigate", "ops_start_kyc");
    } else {
