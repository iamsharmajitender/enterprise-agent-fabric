...
Each run mints a new session and payload ids. Chat-only routes live here; jobs use run-job.sh.
EOF
  exit 2
}
...
expected = sys.argv[2]
if expected:
    sys.exit(0 if status == "completed" and body.get("message") == expected else 1)
sys.exit(0 if status in {"completed", "failed"} else 1)
' "$status_tmp" "$expected"; then
