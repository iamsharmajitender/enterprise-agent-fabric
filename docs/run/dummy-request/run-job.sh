...
json.dump(
    {
        ...
        "label": job["label"],
        "token": token,
        "expected_message": job.get("expected_message"),
    },
    sys.stdout,
)
' "$CATALOG" "$1"
...
' "$status_tmp"
      ...
      msg="$(python3 -c 'import json,sys; b=json.load(open(sys.argv[1])); print((b.get("result") or {}).get("message") or b.get("message") or "")' "$status_tmp")"
      rm -f "$status_tmp"
...
run_one() {
  ...
  local claims_json idempotency_key token autonomy label expected
  ...
  if [[ "$CREATE_ONLY" != "1" ]]; then
    ...
    poll_job "$first" "$claims_json" "$expected"
  fi
}
