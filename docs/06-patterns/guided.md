...
`max_tool_calls` (per stage) caps the inner loop. It is not `max_loop_steps` for the whole run — that budget belongs to [autonomous](autonomous.md).
...
Prompt packs: [prompts](../02-understand/prompts.md).
...
| --- | --- |
| One LLM call, no stages | [single-inference](single-inference.md) |
| Model picks the next step for the **whole** run | [autonomous](autonomous.md) |
| No inner choice; every stage names exactly one tool | [deterministic](deterministic.md) |
