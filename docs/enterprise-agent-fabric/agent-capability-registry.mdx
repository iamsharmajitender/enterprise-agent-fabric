...
| `kind` | `domain` (business API) or `agent_start` (start another catalogue row). |
...
- Treating a published capability as permission. Proposal, then PEP, then AFD entitle for `agent_start`.
