...
```
Specialists / facts (optional kind=agent join)
    │
    ▼
...
1. Extract facts → order / billing / policy specialists (join).  
...
Demo: [`chat/1-autonomous/shopassist_case.sh`](../run/dummy-request/chat/1-autonomous/shopassist_case.sh). Soft path without escalate: [`shopassist_case_no_escalate.sh`](../run/dummy-request/chat/1-autonomous/shopassist_case_no_escalate.sh).
