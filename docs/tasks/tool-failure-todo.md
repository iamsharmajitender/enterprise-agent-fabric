...
Plan: [tool-failure-plan.md](./tool-failure-plan.md). Docs map: [docs/README.md](../README.md). Related: [future-enhancement §4 side-effect invoke](./future-enhancement.md#4-side-effect-tools--invoke-policy), [audit-todo](./audit-todo.md) (optional later emit).
...
**Description:** Document the shared `ToolOutcome` contract (ok, errorCategory, isRetryable, code, customerMessage, developerMessage, data, message) and mapping rules for soft empty vs business/permission vs technical. Add checked JSON examples under `docs/05-reference/`.
...
- `docs/05-reference/tool-outcome-envelope.json` (and/or examples)
- `docs/tasks/tool-failure-plan.md` (link only if needed)
...
- `docs/tasks/tool-failure-plan.md` (open questions → decisions)
...
**Verification:**
- [ ] Automated test or `docs/run` script with clear pass/fail
- [ ] Dummy `--all` not used as the proof

**Dependencies:** TF4, TF6 (TF5 if linear demo chosen)

**Files likely touched:**
- `agent-runtime/tests/**` or `docs/run/dummy-request/**`
- `docs/tasks/tool-failure-plan.md` (demo path checkboxes)
...
**Verification:**
- [ ] Links from [docs/README.md](../README.md) reader path resolve
...
- `docs/02-understand/status.md`
- `docs/tasks/future-enhancement.md`
- `docs/README.md` (if not already linked)
