...
Plan: [audit-plan.md](./audit-plan.md). Docs map: [docs/README.md](../README.md). Parked summary: [future-enhancement.md](./future-enhancement.md#3-audit--decision-record--evidence-chain).
...
**Description:** Freeze the audit event envelope and one example per Phase A `event_type` under `docs/05-reference/`. Document allowlisted payload fields and default-deny (no raw utterance, claims JSON, tokens).
...
- `docs/05-reference/audit-event-envelope.json` (and/or per-type examples)
- `docs/tasks/audit-plan.md` (link only if needed)
...
**Acceptance criteria:**
- [x] `docs/run/compose/init-postgres.sql` creates `audit`
...
- `docs/run/compose/docker-compose.yml` (labels/comments if any)
...
- `docs/run/compose/docker-compose.yml` (service stub may wait for A4)
...
- [x] `./docs/run/scripts/start-app.sh` brings AADP + AACP up (ingest may still be stub)
...
- `agent-audit-data-plane/src/main/java/**`
- `agent-audit-data-plane/src/test/java/**`
- `docs/05-reference/` (optional response examples)
...
- `agent-runtime/tests/**`
- `docs/05-reference/` (stage event example)
...
**Acceptance criteria:**
- [x] Doc section in AADP README or `docs/02-understand/` / plan appendix: retention default (e.g. 30d local), deny-list, hash fields
...
- `docs/tasks/audit-plan.md` (pointer)
...
- `docs/tasks/audit-plan.md` (durability note)
