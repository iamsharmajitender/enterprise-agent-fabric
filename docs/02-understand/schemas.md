...
| `input_schema` | HTTP tool request body (later: hop validation) | Catalogue and UI today. Runtime does not validate HTTP against it yet. See [data](data.md) |
...
Seed example: [`extract_fields`](http://localhost:3006/capabilities/extract_fields) (`purchase_refund` classify). Frozen copy: [`../05-reference/capability-extract-fields.json`](../05-reference/capability-extract-fields.json).
...
Do not duplicate the key list in the prompt when the schema already names them. See [prompts](prompts.md).
