...
| 1 Autonomous | `1` | Manifest + `max_loop_steps`. No workflow | LLM `CALL` / `DONE` loop. `CALL` runs a domain tool, then the LLM runs again, up to `max_loop_steps` |
...
Intended compositions (what is possible, including rows not in the seed): [06-patterns](../06-patterns/). Examples in this binary’s seed: Pattern 0 `email_summarize`; Pattern 1 `fee_explain`; Pattern 2 `llm_pipeline`, `card_freeze`, `policy_memo`. Catalogue matrix: [03-catalogue](../03-catalogue/).
