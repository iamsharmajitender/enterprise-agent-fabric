...
HTTP merge: `goal` ∪ prior-slot keys that appear on the **next** capability `input_schema` — see [data](data.md).
...
Retrieval is a sibling table. Prefetch writes `working.slots.prefetch`; it is not `long_term`. See [retrieve](retrieve.md).
