...
After mint, this use case is identical to [files-via-dms](files-via-dms.md). The domain tool GETs the DMS. Runtime and the LLM never see bytes.
...
Sibling of `retrieval` and `memory_profile` on the route. Not a fifth `autonomy_mode`. Example composition: [`msa_risk_review`](../03-catalogue/routes.md#msa_risk_review) would keep Pattern 2, retrieval, memory, and add this block.
...
| Shape | Where |
| --- | --- |
| Ids or presigned GET already in JSON | [files-via-dms](files-via-dms.md) |
