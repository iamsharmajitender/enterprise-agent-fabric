...
Plan: [observability-plan.md](./observability-plan.md). Docs map: [README.md](../README.md). Blueprint: [Observability: Three Layers, One Graph](https://jitendersharma.dev/blueprints/observability-blueprint).
...
**Verification:**
- [x] `docker compose -f agent-fabric-scripts/docker-compose/docker-compose.yml config` shows OTEL vars on app services
- [x] `./agent-fabric-scripts/stack/start-app.sh` still brings stack up; `curl -sf localhost:3005/health` (and siblings) succeed
...
**Verification:**
- [x] `docker compose -f agent-fabric-scripts/docker-compose/docker-compose.yml logs agent-front-door` shows parseable JSON (or documented format)
...
- `agent-fabric-scripts/docker-compose/docker-compose.yml` (any extra Java OTEL agent env if used)
...
**Files likely touched:**
- `agent-fabric-scripts/docker-compose/docker-compose.yml`
- `README.md` or `agent-fabric-docs/tasks/observability-plan.md` note
