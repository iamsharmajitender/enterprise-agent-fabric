...
Plan: [plan.md](./plan.md). Docs map: [README.md](../README.md).
...
- [x] `agent-fabric-docs/05-reference/stub-auth.md` defines channel `Bearer stub` + `X-Stub-Claims` and workload `Bearer fabric-internal` + `X-Workload`
- [x] `agent-fabric-scripts/docker-compose/.env.example` lists DB URLs for the four services, HTTP ports 3005 / 3007 / 3008 / 3009, and Adminer 8080

**Verification:**
- [x] `docker compose -f agent-fabric-scripts/docker-compose/docker-compose.yml exec postgres psql -U fabric -d afd -c '\l'` shows `afd`, `adp`, `ar`, `acr` only (plus Postgres templates; no `acp`)
...
- `agent-fabric-docs/05-reference/stub-auth.md`

**Estimated scope:** Small

---

## Task 2: Pack JSON fixtures under `agent-fabric-docs/05-reference/`
...
**Verification:**
- [x] Manual check: files match pack examples in `agent-fabric-docs/04-architecture/`

**Dependencies:** None

**Files likely touched:**
- `agent-fabric-docs/05-reference/decide-chat-route.json`
- `agent-fabric-docs/05-reference/assistant-turn-request.json`
- `agent-fabric-docs/05-reference/run-start.json`
- `agent-fabric-docs/05-reference/capability-account-fee-lookup.json`
- `agent-fabric-docs/05-reference/manifest-fee-explain.json`
...
**Verification:**
- [x] `docker compose -f agent-fabric-scripts/docker-compose/docker-compose.yml up agent-capability-registry` then `curl -sf localhost:3009/health` → `{"status":"UP"}`
...
- [x] Manual: GET matches `agent-fabric-docs/05-reference/manifest-fee-explain.json`
...
- [x] Manual: POST body from `agent-fabric-docs/05-reference/decide-chat-route.json`
...
- `agent-fabric-docs/05-reference/decide-jobs-route.json`
- `agent-fabric-docs/05-reference/jobs-start.json`
...
**Acceptance criteria:**
- [x] `agent-fabric-scripts/route-runs/job/1-autonomous/fee_explain.sh` proves `POST /v1/jobs` for `fee_explain` and GET until the canned fee message
...
**Files likely touched:**
- `agent-fabric-scripts/route-runs/job/1-autonomous/fee_explain.sh`
- `agent-fabric-front-door/README.md`
...
**Description:** `agent-fabric-runtime/README.md` explains start, hydrate, LangGraph stub graph, run pin schema, uv commands.
...
**Description:** `agent-fabric-front-door/README.md` adds chat AFD: hints, turns, events poll, freeze table, FR-5, sibling call map. Jobs routes are already documented in Task 31.
...
- [x] `./agent-fabric-scripts/route-runs/run-job.sh fee_explain` and `./agent-fabric-scripts/route-runs/run-chat.sh fee_explain` pass on a clean compose
