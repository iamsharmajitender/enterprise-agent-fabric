...
**Confirmed intent:** [intent/enterprise-agent-fabric-v1.md](../intent/enterprise-agent-fabric-v1.md)
...
- **Five processes, four databases, one Compose file.** `agent-fabric-scripts/docker-compose/docker-compose.yml`. One Postgres 16 server, databases `afd`, `adp`, `ar`, `acr`. No `acp` database. HTTP: 3005 Front Door, **3006 Control Plane UI**, 3007 Data Plane, 3008 AR, 3009 Registry.
...
- [x] Task 2: Pack JSON fixtures under `agent-fabric-docs/05-reference/`
...
- [x] `docker compose -f agent-fabric-scripts/docker-compose/docker-compose.yml up` — `/health` 200 on 3005, 3007, 3008, 3009; Control Plane process running with no published port; Adminer on 8080
...
- [ ] Task 28: `agent-fabric-runtime/README.md`
- [ ] Task 29: `agent-fabric-front-door/README.md` (chat APIs; jobs already in Task 31)
- [ ] Task 24: Root README (compose, ports, jobs + chat demos, links to service handbooks)

### Checkpoint: Complete

- [ ] All acceptance criteria in `agent-fabric-docs/tasks/todo.md` through Task 24 met
...
| --- | --- | --- |
| Three Java copies drift | Med | Same hexagonal Spring/Flyway layout; contract fixtures in `agent-fabric-docs/05-reference/` are the shared source |
