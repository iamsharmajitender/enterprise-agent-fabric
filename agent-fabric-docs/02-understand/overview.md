...
What Runtime passes between stages: [data](data.md). Capability JSON Schema (`input_schema` / `output_schema`): [schemas](schemas.md). Capability kinds (`domain` vs `agent`): [capabilities](capabilities.md). Catalogue vs Runtime: [status](status.md). Who mints `request_id` / `session_id` / `correlation_id` (and Grafana queries): [ids](ids.md).
