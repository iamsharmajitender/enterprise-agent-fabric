...
- **human_gate** — branch routes pick the next stage from a prior slot; `human_gate` sets `status=waiting` until `POST /v1/runs/{id}/turns` merges a human packet. See [patterns](patterns.md) and [human-review-process-gate](../07-usecases/human-review-process-gate.md).
...
- **escalate_to_human** — Pattern 1 domain tool that opens an async handoff; parent completes when the handoff succeeds and no `kind=agent` joins are pending; repeat CALL does not create another ticket. See [escalate-to-human-handoff](../07-usecases/escalate-to-human-handoff.md). Not a `human_gate`.
