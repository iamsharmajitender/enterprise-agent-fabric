...
**`escalate_to_human`:** after a successful handoff, Runtime auto-completes the parent when no pending `kind=agent` joins remain; a second `CALL escalate_to_human` is idempotent (same handoff, no second POST). See [escalate-to-human-handoff](../07-usecases/escalate-to-human-handoff.md).
