...
Executive quick deck: [Executive quick deck](/architecture/executive-briefs/quick-deck) · Technical design: [Enterprise Agent Fabric](/architecture/technical-design)
...
Instead of every domain shipping another AI application with its own front door, identity model, and controls, the Enterprise Agent Fabric provides a **shared operating layer** between users and specialised AI capabilities.

Users and applications ask the enterprise for help. The Fabric handles **who** may act, **which** capability should respond, and **how** that work runs under common policy, audit, and operations — while each domain keeps ownership of its specialised workflows, knowledge, and tools.

Every request follows the same governed path:

1. **One experience** — one enterprise entry point for employees, customers, applications, and jobs
2. **Governed decision** — intent understood, the right capability selected, entitlement and policy verified
3. **Specialised capability** — domain-owned AI executes with approved data and tools
4. **Controlled action** — high-risk steps gated by policy, approval, or human handoff
5. **Auditable outcome** — evidence of what was decided, executed, and approved, recorded consistently

The Fabric is not one giant agent. It is the enterprise infrastructure that makes many governed capabilities feel like one capability to the organisation.
...
This is a proof point for the architecture and operating model, not the final production topology. Implementation status and behaviour truth: [status](/catalogue/coverage-status) · [catalogue matrix](/catalogue/) · [technical design](/architecture/technical-design).
