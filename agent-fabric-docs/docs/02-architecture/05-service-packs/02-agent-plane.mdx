...
description: "Catalogue and classify. The Agent Control Plane and Agent Data Plane sit here. Neither pins a route nor starts Runtime."
---
...
The Agent Plane is the catalogue and the classifier. There is one plane per trust domain — not one mega-agent for the enterprise.

Two boxes sit inside it:

- **Agent Data Plane** — routes, prompts, manifests, workflows. The [Front Door](/architecture/service-packs/agent-front-door) calls it to decide.
- **Agent Control Plane** — the catalogue UI. Locally it has no database and no decide API.

Neither box pins a route. Neither box starts [Runtime](/architecture/service-packs/agent-runtime). The [Agent Capability Plane](/architecture/service-packs/agent-capability-registry) sits beside them and shares the same store.

## What it is

The Data Plane answers: given this caller, this channel, and this utterance (or named `route_id`), which governed path owns the turn? Outcomes are `route`, `clarify`, or `abstain`. Only `route` is startable, and it is the Front Door that starts.

## What it owns

| Owns | Does not own |
| --- | --- |
| Route catalogue, eligible set, decide | The freeze, the run pin |
| Prompt packs, workflow pointers | LangGraph execution |
| Control UI over the catalogue | Channel auth, public ingress |

## What it must not do

- Pin `route_id` or start Runtime.
- Expose a public URL. Only Front Door calls decide.
- Collapse Control Plane and Data Plane into one process that both classifies and executes.

## How to talk to it

| Method | Path | Who |
| --- | --- | --- |
| `POST` | `/v1/intent/decide` | Front Door only |
| `GET` | `/v1/intent/eligible` | Front Door, Control Plane |
| `GET` | `/v1/catalog/routes` | Front Door, Control Plane, Runtime (pinned version) |

Fixtures: [decide-chat-request.json](/fixtures/decide-chat-request.json).
...
<a href={useBaseUrl('/diagrams/plane-solution.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
Frozen route/session is on the [AFD](/architecture/service-packs/agent-front-door#frozen-route-session). Run pin is on [AR](/architecture/service-packs/agent-runtime). Capability versions are on the [Agent Capability Plane](/architecture/service-packs/agent-capability-registry) (same Agent Plane band).
...
![Cheap layers first; safety on every path. Layer ① bind, Layer ② retrieve, Layer ③ only if needed. Jobs skip Layer ② and ③ but still entitle and record.](/diagrams/agent-fabric-classifier.svg)
...
AFD channel APIs: [Agent Front Door §6](/architecture/service-packs/agent-front-door#apis).
...
**Local seed:** route packs under `agent-fabric-scripts/stack/route/`. See [author an agent](/guides/author-an-agent).
...
<a href={useBaseUrl('/diagrams/plane-seq-decide.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
Scale Chat AFD, API AFD, and Agent Plane as three fleets. Decide must not wait on a 12-step loop. Numbers: [Enterprise Agent Fabric §3](/architecture/technical-design#nfrs).
...
AFD and AR scale: [Agent Front Door §12](/architecture/service-packs/agent-front-door#scaling) · [Agent Runtime §12](/architecture/service-packs/agent-runtime#scaling).

---

## Read next

**[Agent Runtime](/architecture/service-packs/agent-runtime)** · [Agent Capability Plane](/architecture/service-packs/agent-capability-registry) · [Agent Front Door](/architecture/service-packs/agent-front-door) · [Autonomy](/autonomy/)
