...
Chat AFD is BFF-shaped (Chat/UI). API AFD is handler-shaped (HTTP jobs + Kafka ingest). They share [Agent Fabric Plane](/architecture/service-packs/agent-plane), the frozen-route/session store, and the start contract. They do not share a process (FR-9). Kafka `{runs_topic}` is [AR](/architecture/service-packs/agent-runtime) return, not a third AFD.
...
Decide JSON: [Agent Fabric Plane](/architecture/service-packs/agent-plane#apis). Start / `202`: [Agent Fabric Runtime](/architecture/service-packs/agent-runtime#apis).
...
Plane and AR scale: [Agent Fabric Plane §12](/architecture/service-packs/agent-plane#scaling) · [Agent Fabric Runtime §12](/architecture/service-packs/agent-runtime#scaling).

---

## Read next

**[Agent Fabric Plane](/architecture/service-packs/agent-plane)** · [Agent Fabric Runtime](/architecture/service-packs/agent-runtime) · [Agent Fabric Capability Registry](/architecture/service-packs/agent-capability-registry) · [Enterprise Agent Fabric](/architecture/technical-design)
