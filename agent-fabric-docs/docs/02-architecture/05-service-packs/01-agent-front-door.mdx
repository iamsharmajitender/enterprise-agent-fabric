---
title: "Agent Front Door"
sidebar_label: Agent Front Door
description: "The only public way into the Fabric. Entitles the caller, freezes the route, and starts Runtime. Chat and jobs share one contract."
---
...
The Front Door is the only public way into the Fabric. Chat, jobs, and partner APIs all stop here. It checks who you are, freezes which agent will run, and starts [Agent Runtime](/architecture/service-packs/agent-runtime). It never classifies a request itself and it never runs the agent.

Try it: [your first request](/guides/first-request).

## What it is

Two shapes, one contract: **entitle → classify (chat) or entitle-only (jobs) → freeze → start**.

- **Chat** is a BFF. Humans send an utterance. The Front Door asks the [Agent Data Plane](/architecture/service-packs/agent-plane) to classify. Only `outcome=route` starts Runtime.
- **Jobs** are an API. The caller already named `route_id`. Classification is skipped; entitlement is not.

Locally both shapes share one Front Door process. At production scale they are separate fleets so a hung chat pool cannot take down jobs. They still share the Agent Plane, the freeze store, and the start contract.

## What it owns

| Owns | Does not own |
| --- | --- |
| Channel auth, `session_id`, hints, SSE/poll, jobs API | Classify, eligible set, route catalogue |
| Frozen route/session, pin, async start | The run pin, the LangGraph loop |
| Fan-out of Runtime status | Agent identity mint, tool invoke |

## What it must not do

- Classify. That is the Data Plane.
- Start Runtime on `clarify` or `abstain`.
- Let a channel dial Runtime or the Agent Plane.
- Show the chat UI a `route_id`, `run_id`, or `agent_client_id`.

## How to talk to it

| Method | Path | Who |
| --- | --- | --- |
| `POST` | `/v1/assistant/turns` | Chat |
| `GET` | `/v1/assistant/hints` | Chat empty state |
| `POST` | `/v1/jobs` | Jobs, partners, parent agents |
| `GET` | `/v1/jobs/{correlation_id}` | Jobs poll |

Headers: [stub auth](/reference/stub-auth). Fixtures: [reference](/reference/).

---

## Design notes

The sections below are the internals of this box — fleets, freeze record, APIs, and scale.
...
<a href={useBaseUrl('/diagrams/afd-solution.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
<a href={useBaseUrl('/diagrams/afd-context.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
The AFD stores the **frozen route/session** so `"yes"` or a jobs retry keeps the same `route_id`, versions, and `agent_client_id`. Pin miss: **AR API**, not AR Postgres. See [Three principals](/architecture/technical-design#three-principals).
...
Decide JSON: [Agent Plane](/architecture/service-packs/agent-plane#apis). Start / `202`: [Agent Runtime](/architecture/service-packs/agent-runtime#apis).
...
<a href={useBaseUrl('/diagrams/afd-seq-ui.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
<a href={useBaseUrl('/diagrams/afd-seq-jobs.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
Decide capacity is turns/s × (entitle + classify + pin + start-ack). Run capacity is a different fleet. Scale Chat AFD, API AFD, and Agent Plane as three fleets (FR-9). Numbers: [Enterprise Agent Fabric §3](/architecture/technical-design#nfrs).
...
Plane and AR scale: [Agent Plane §12](/architecture/service-packs/agent-plane#scaling) · [Agent Runtime §12](/architecture/service-packs/agent-runtime#scaling).

---

## Read next

**[Agent Plane](/architecture/service-packs/agent-plane)** · [Agent Runtime](/architecture/service-packs/agent-runtime) · [Agent Capability Plane](/architecture/service-packs/agent-capability-registry) · [Enterprise Agent Fabric](/architecture/technical-design)
