...
description: "Where the agent actually runs. Pins the freeze, hydrates capabilities, and executes the graph. Does not classify the next utterance."
---
...
Runtime is where the agent does the work. Only the [Front Door](/architecture/service-packs/agent-front-door) starts it. It copies the freeze onto a durable **run pin**, hydrates every capability on that pin, returns `202` with a `correlation_id`, and runs the graph.

It does not classify the next utterance. Memory, RAG, model, and tools sit in **Shared** and are called from here, not built in.

## What it is

Three steps, always in this order:

1. **Pin** — copy `route_id@version` from the freeze. Do not re-read `active`.
2. **Hydrate** — resolve the catalogue row and every `id@version` from the [Agent Capability Plane](/architecture/service-packs/agent-capability-registry).
3. **Run** — compile the LangGraph for the route's [autonomy mode](/concepts/executing-a-request/autonomy-modes).

The platform owns the shell (pin, hydrate, graph, telemetry). The domain owns the agent (prompts, capabilities, judgement).

## What it owns

| Owns | Does not own |
| --- | --- |
| Run pin, hydrate snapshot, graph loop | Classify, eligible set |
| Calls into Shared (memory, RAG, tools) | Channel auth, public ingress |
| `correlation_id` on `202` | The freeze store |

## What it must not do

- Classify or start itself.
- Re-read the catalogue or registry mid-loop.
- Use the user token as the tool-call caller. User, agent, and Runtime workload are [three principals](/architecture/technical-design#three-principals).

## How to talk to it

Channels never see this URL. Front Door calls:

| Method | Path |
| --- | --- |
| `POST` | `/v1/runs` |
| `POST` | `/v1/runs/{id}/turns` |
| `GET` | `/v1/runs/{id}` |

Fixtures: [run-start.json](/fixtures/run-start.json).
...
<a href={useBaseUrl('/diagrams/runtime-solution.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
Patterns 0–3 answer **who decides the next step** inside AR. Full contracts: [autonomy](/autonomy/).
...
Detail: [generic shapes](/use-cases/generic-shapes).
...
Who calls start: [Agent Front Door call map](/architecture/service-packs/agent-front-door#call-map).

---

## 6. Shared

**Memory**, **RAG**, and **Tools** sit in Shared. The **Agent Capability Plane** sits with Agent Plane. AR calls them. They are not inside AR.

After freeze, AR **downloads the whole pinned manifest**, hydrates each `id@version` onto the run pin, then sends schemas only to the LLM. Domain `invoke` hits a governed business API. Agent-start `invoke` hits the API AFD jobs URL. [Agent Capability Plane](/architecture/service-packs/agent-capability-registry).
...
<a href={useBaseUrl('/diagrams/runtime-seq-start.html')} target="_blank" rel="noreferrer">Editorial source</a>
...
AFD and Plane scale: [Agent Front Door §12](/architecture/service-packs/agent-front-door#scaling) · [Agent Plane §12](/architecture/service-packs/agent-plane#scaling).

---

## Read next

**[Agent Capability Plane](/architecture/service-packs/agent-capability-registry)** · [Agent Front Door](/architecture/service-packs/agent-front-door) · [Agent Plane](/architecture/service-packs/agent-plane) · [Patterns](/autonomy/) · [Runtime status](/catalogue/coverage-status)
