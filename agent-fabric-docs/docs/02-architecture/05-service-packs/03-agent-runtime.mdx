...
Only the **AFD** starts AR. On `route`, that AFD `POST`s the freeze. AR **fetches the catalogue row** at the pinned version, **hydrates** from the Registry, copies the **run pin**, returns `202 { "correlation_id" }`, and runs LangGraph. Memory, RAG, and Tools sit in **Shared**. Fabric rule: [Three principals](/architecture/technical-design#three-principals).

**Local v1:** Python FastAPI + LangGraph on port **3008**. Patterns 0–3, `human_gate`, `kind=agent`, checkpoint resume wired. See [status](/catalogue/coverage-status) for gaps.
...
Who calls start: [Agent Fabric Front Door call map](/architecture/service-packs/agent-front-door#call-map).
...
After freeze, AR **downloads the whole pinned manifest**, hydrates each `id@version` onto the run pin, then sends schemas only to the LLM. Domain `invoke` hits a governed business API. Agent-start `invoke` hits the API AFD jobs URL. [Agent Fabric Capability Registry](/architecture/service-packs/agent-capability-registry).
...
AFD and Plane scale: [Agent Fabric Front Door §12](/architecture/service-packs/agent-front-door#scaling) · [Agent Fabric Plane §12](/architecture/service-packs/agent-plane#scaling).

---

## Read next

**[Agent Fabric Capability Registry](/architecture/service-packs/agent-capability-registry)** · [Agent Fabric Front Door](/architecture/service-packs/agent-front-door) · [Agent Fabric Plane](/architecture/service-packs/agent-plane) · [Patterns](/autonomy/) · [Runtime status](/catalogue/coverage-status)
