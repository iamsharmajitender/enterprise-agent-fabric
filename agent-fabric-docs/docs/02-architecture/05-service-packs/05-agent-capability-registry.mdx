---
title: "Agent Capability Plane"
sidebar_label: Agent Capability Plane
description: "Publishers append id@version. Routes refer. Runtime hydrates once at pin. Not a client the loop calls on every turn."
---
...
Publishers append a capability as `id@version`. A route refers to those versions. [Runtime](/architecture/service-packs/agent-runtime) hydrates the whole pinned manifest **once**, at pin, then never re-reads the registry mid-loop.

This box sits with the [Agent Plane](/architecture/service-packs/agent-plane), beside Control Plane and Data Plane. It is a catalogue of tools and child agents — not a third Front Door, not MCP `list_tools`, and not Shared Tools.

## What it is

Two kinds of capability:

- **domain** — a governed business API the agent may invoke.
- **agent** — start another catalogue route through the Front Door jobs API.

Kinds and schemas: [capabilities](/concepts/authoring-a-product/capability).

## What it owns

| Owns | Does not own |
| --- | --- |
| Immutable `id@version` rows and manifests | Classify, pin, start |
| Hydrate reads at the pinned version | Mid-loop tool dispatch |

## What it must not do

- Edit a published version. A fix is a new version.
- Be called from the LangGraph loop on every tool turn.
- Live in Shared. Memory, RAG, and Tools stay there.

## How to talk to it

Runtime GETs at pin. Publishers PUT a version. Fixtures: [capability-account-fee-lookup.json](/fixtures/capability-account-fee-lookup.json), [manifest-fee-explain.json](/fixtures/manifest-fee-explain.json).
