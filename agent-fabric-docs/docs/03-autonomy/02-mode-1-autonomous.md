---
title: "Mode 1 — Autonomous"
sidebar_label: "Mode 1 — Autonomous"
description: "autonomy_mode 1. The LLM picks the next step from a hydrated manifest in a CALL/DONE loop bounded by max_loop_steps."
---
...
```
goal  →  LLM (CALL, ASK, or DONE)
              │
              ├─ CALL capability [optional JSON args]  →  HTTP (schema keys only)  →  observe  →  LLM again
              ├─ ASK question  →  waiting_for=customer_ask  →  customer reply  →  LLM again
              └─ DONE  →  answer
```
...
Front Door POST /v1/runs
   ...
   graph    build_agent_loop (agent-runtime/app/core/agent_core.py, mode == 1)
        ...
        ▼
   each step
            llm.complete → CALL <id> [JSON args]  →  _run_stage HTTP (llm_role forced none)
                         → ASK <question>  →  pause waiting_for=customer_ask
                         → DONE <answer>  →  result
...
Data that moves: HTTP payload is `dict(goal)` ∪ schema-selected **slots**; LLM reads **notes**. See [data](/concepts/executing-a-request/run-data).
...
Front Door starts. Runtime pins (no workflow). Catalogue hydrates the manifest. The LLM **CALL**s or **DONE**s; CALL hits domain HTTP and notes loop back. <a href={useBaseUrl('/diagrams/pattern-1-swimlane.html')} target="_blank" rel="noreferrer">Open as a page</a>.
...
| Message | Source |
| --- | --- |
| **system** | Fixed contract (`CALL` / `ASK` / `DONE`), plus pack `host` (same extra text every turn, from `prompt_id`), plus `Tools: id1, id2, …` from the hydrated manifest. |
| **user** | `_user_blob`: `goal: {…}` and `prior stage outputs:` (tool strings accumulated so far). |

The model replies with **one** of:

- `CALL <tool_id>` plus optional JSON object of `input_schema` fields — lookup that id on the pin, force `llm_role=none`, POST schema keys only to `invoke.url`, append `text`/`message` to `notes`, loop.
- `ASK <question>` — pause (`waiting_for=customer_ask`). Chat `/turns` `{message}` appends `customer: …` and continues the same goal.
- `DONE <answer>` — that answer is `result`. Loop ends. On_stage records this as `respond`.

Unknown tool id fails the run. Exceeding `max_loop_steps` fails the run. A retrieve capability is just another CALL. Prefetch is not a CALL. Pattern 1 uses `host` only — it does not stamp `by_llm_role` onto tools ([prompts](/concepts/authoring-a-product/prompts)).

**`escalate_to_human`:** after a successful handoff, Runtime auto-completes the parent when no pending `kind=agent` joins remain; a second `CALL escalate_to_human` is idempotent (same handoff, no second POST). See [escalate-to-human-handoff](/use-cases/escalate-to-human-handoff).
...
| --- | --- |
| One LLM call, no tools | [single-inference](/autonomy/mode-0-single-inference) |
| Fixed stage order, even if stages call tools | [deterministic](/autonomy/mode-2-deterministic) |
| Fixed stages, LLM chooses tools **inside** a stage | [guided](/autonomy/mode-3-guided) |
