---
title: "Mode 0 — Single inference"
sidebar_label: "Mode 0 — Single inference"
description: "autonomy_mode 0. Nobody picks the next step: one LLM call from a prompt pack, with optional prefetch and memory around it."
---

import useBaseUrl from '@docusaurus/useBaseUrl';

# Mode 0 — Single inference
...
Front Door starts. Runtime pins. Data Plane returns the pack. Runtime makes **one** synthesis call. <a href={useBaseUrl('/diagrams/pattern-0-swimlane.html')} target="_blank" rel="noreferrer">Open as a page</a>.
...
`llm_role=synthesis` skips HTTP even if a url were present. Pattern 0 also has empty `invoke`, so there is no domain POST. Prefetch is packing around this call, not a retrieve tool. Memory is extra **user** lines, not a second node. Prompt packs: [prompts](/concepts/authoring-a-product/prompts).
...
Escalate to [autonomous](/autonomy/mode-1-autonomous) when the model must **choose** a tool. Escalate to [deterministic](/autonomy/mode-2-deterministic) when you want a **fixed stage list** (several LLM stages, or prefetch as an explicit `none` stage then `synthesis`).
...
| --- | --- |
| Several LLM stages in a fixed order, still no tools | [deterministic](/autonomy/mode-2-deterministic) LLM-only (`llm_pipeline`, `policy_memo`) |
| Model chooses a retrieve or domain tool | [autonomous](/autonomy/mode-1-autonomous) |
| Fixed stages, model chooses tools inside a stage | [guided](/autonomy/mode-3-guided) |
