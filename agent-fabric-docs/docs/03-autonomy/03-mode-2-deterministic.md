---
title: "Mode 2 — Deterministic"
sidebar_label: "Mode 2 — Deterministic"
description: "autonomy_mode 2. The workflow designer picks the next step. A fixed stage list; the LLM only does the work a stage assigns."
---
...
Front Door starts. Runtime pins a workflow. Catalogue stamps `llm_role`. Runtime walks a **linear** graph: each stage completes with the LLM or calls domain HTTP. <a href={useBaseUrl('/diagrams/pattern-2-swimlane.html')} target="_blank" rel="noreferrer">Open as a page</a>.
...
Pattern 0 composition 2 vs this pattern’s prefetch-then-generate: both pack then call the model. Pattern 0 is packing **around** one synthesis node. Pattern 2 is an explicit stage list you can extend with more fixed LLM or HTTP stages. Prompt packs: [prompts](/concepts/authoring-a-product/prompts).
...
Same four attachments as [single-inference](/autonomy/mode-0-single-inference), but **N LLM calls in a fixed order**.
...
| --- | --- |
| One LLM call, no stage list | [single-inference](/autonomy/mode-0-single-inference) |
| Model chooses the next tool | [autonomous](/autonomy/mode-1-autonomous) |
| Model chooses tools **inside** a fixed stage | [guided](/autonomy/mode-3-guided) |
