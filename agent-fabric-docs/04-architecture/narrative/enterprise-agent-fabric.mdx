...
8. **Capabilities are references.** Publishers append `id@version` next to Control Plane and Data Plane. Manifests refer. NAR hydrates the whole pinned manifest before the LLM. `kind=agent` `invoke` is API NAFD jobs, not the callee NAR.
