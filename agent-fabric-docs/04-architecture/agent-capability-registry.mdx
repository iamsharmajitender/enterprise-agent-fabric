...
description: "Agent Capability Registry: publishers append id@version. Agent developers refer from the tool manifest. AR hydrates at pin. Domain APIs and agent-start invoke go through PEP."
---
...
Parent: [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx) · Siblings: [Agent Front Door](./agent-front-door.mdx) · [Agent Plane](./agent-plane.mdx) · [Agent Runtime](./agent-runtime.mdx)
...
The registry sits **with Agent Plane** (beside ACP and Data Plane). Memory, RAG, and Tools / PEP stay in Shared. It is a catalog, not a third AFD, not MCP `list_tools`, and not a client the loop calls on every turn.
...
**Same freeze vs new route.** A summarizer helper is a **domain** capability on this manifest. Legal MSA review is an **agent-start** capability. The second one's `invoke` is the [API AFD](./agent-front-door.mdx) jobs URL, not Legal's `activation_target`.
...
AR start still goes to `{activation_target}` from [Runtime §5](./agent-runtime.mdx#apis). Agent-start `invoke` is a later `POST` to `{jobs_url}` on [API AFD](./agent-front-door.mdx#apis).
...
**[Agent Runtime](./agent-runtime.mdx)** · [Agent Front Door](./agent-front-door.mdx) · [Agent Plane](./agent-plane.mdx) · [Capabilities](../02-understand/capabilities.md) · [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx)
