...
![Enterprise Agent Fabric target state (high-level architecture): Agent security and Agent evals on the left; Events at the top; Front Door, Plane, Runtime, and Shared left to right; Observability and Agent audit side by side feeding Dashboard then Responsible / Governed AI.](./diagrams/agent-fabric-architecture.svg)
