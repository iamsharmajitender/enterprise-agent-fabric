...
Parent: [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx) · Siblings: [Agent Plane](./agent-plane.mdx) · [Agent Runtime](./agent-runtime.mdx) · [Agent Capability Registry](./agent-capability-registry.mdx)
...
Chat AFD is BFF-shaped (Chat/UI). API AFD is handler-shaped (HTTP jobs + Kafka ingest). They share [Agent Plane](./agent-plane.mdx), the frozen-route/session store, and the start contract. They do not share a process (FR-9). Kafka `{runs_topic}` is [AR](./agent-runtime.mdx) return, not a third AFD.
...
Decide JSON: [Agent Plane](./agent-plane.mdx#apis). Start / `202`: [Agent Runtime](./agent-runtime.mdx#apis).
...
Plane and AR scale: [Agent Plane §12](./agent-plane.mdx#scaling) · [Agent Runtime §12](./agent-runtime.mdx#scaling).

---

## Read next

**[Agent Plane](./agent-plane.mdx)** · [Agent Runtime](./agent-runtime.mdx) · [Agent Capability Registry](./agent-capability-registry.mdx) · [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx)
