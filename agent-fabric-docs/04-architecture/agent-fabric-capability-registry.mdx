...
sidebar_label: Agent Fabric Capability Registry
sidebar_position: 4
slug: /enterprise-agent-fabric/agent-fabric-capability-registry
...
Parent: [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx) · Siblings: [Agent Fabric Front Door](./agent-fabric-front-door.mdx) · [Agent Fabric Plane](./agent-fabric-plane.mdx) · [Agent Fabric Runtime](./agent-fabric-runtime.mdx) · [Agent Fabric Audit](./agent-fabric-audit.mdx)
...
**Local v1:** Java service on port **3009**. Hydrate is real at pin. Capability kinds: [capabilities](../02-understand/capabilities.md).
...
[Editorial source](./diagrams/registry-solution.html)
...
**Same freeze vs new route.** A summarizer helper is a **domain** capability on this manifest. Legal MSA review is an **agent-start** capability. The second one's `invoke` is the [API AFD](./agent-fabric-front-door.mdx) jobs URL, not Legal's `activation_target`.
...
Identity: jobs `Authorization` is the **calling** agent token. After freeze, Legal mints **its** robot. [Three principals](./enterprise-agent-fabric-architecture.mdx#three-principals).
...
AR start still goes to `{activation_target}` from [Runtime §5](./agent-fabric-runtime.mdx#apis). Agent-start `invoke` is a later `POST` to `{jobs_url}` on [API AFD](./agent-fabric-front-door.mdx#apis).
...
[Editorial source](./diagrams/registry-seq-hydrate.html)
...
[Editorial source](./diagrams/registry-seq-a2a.html)
...
**[Agent Fabric Runtime](./agent-fabric-runtime.mdx)** · [Agent Fabric Front Door](./agent-fabric-front-door.mdx) · [Agent Fabric Plane](./agent-fabric-plane.mdx) · [Agent Fabric Audit](./agent-fabric-audit.mdx) · [Capabilities](../02-understand/capabilities.md) · [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx)
