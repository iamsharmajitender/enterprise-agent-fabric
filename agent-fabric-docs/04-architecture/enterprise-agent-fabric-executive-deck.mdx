...
Security, audit, and observability run **alongside** the request. They do not slow the user unless policy requires a pause or approval.

<div className="gain-diagram-wrap">

![End-to-end request path: message in, classify, entitlement, pin, execute, response out.](./diagrams/agent-fabric-pitch-lifecycle.svg)

</div>

**In one line:** ask → verify and route → specialist AI runs → control if needed → answer or action → evidence retained.
