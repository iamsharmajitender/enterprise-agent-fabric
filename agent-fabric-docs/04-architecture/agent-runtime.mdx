...
description: "Agent Runtime: specialised agents underneath. Run pin, LangGraph loop, Patterns 0-3. Calls Shared for Memory, RAG, and Tools. Hydrates from the Agent Capability Registry."
---
...
Parent: [Enterprise Agent Fabric](./enterprise-agent-fabric-architecture.mdx) · Siblings: [Agent Front Door](./agent-front-door.mdx) · [Agent Plane](./agent-plane.mdx) · [Agent Capability Registry](./agent-capability-registry.mdx)
...
Who calls start: [Agent Front Door call map](./agent-front-door.mdx#call-map).

---

## 6. Shared

**Memory**, **RAG**, and **Tools** sit in Shared. The **Agent Capability Registry** sits with Agent Plane. AR calls them. They are not inside AR.

After freeze, AR **downloads the whole pinned manifest**, hydrates each `id@version` onto the run pin, then sends schemas only to the LLM. Domain `invoke` hits a governed business API. Agent-start `invoke` hits the API AFD jobs URL. [Agent Capability Registry](./agent-capability-registry.mdx).
...
AFD and Plane scale: [Agent Front Door §12](./agent-front-door.mdx#scaling) · [Agent Plane §12](./agent-plane.mdx#scaling).

---

## Read next

**[Agent Capability Registry](./agent-capability-registry.mdx)** · [Agent Front Door](./agent-front-door.mdx) · [Agent Plane](./agent-plane.mdx) · [Patterns](../06-patterns/README.md) · [Runtime status](../02-understand/status.md)
