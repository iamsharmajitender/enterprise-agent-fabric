...
def _hydrate_without_manifest(catalogue: CataloguePort, row: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if prompt_id:
        return _prepend_prefetch_stage(_hydrate_prompt_only(catalogue, prompt_id), row)
...
def _prepend_prefetch_stage(
    tools: list[dict[str, Any]], row: dict[str, Any]
) -> list[dict[str, Any]]:
    """Pattern 0 + deterministic_prefetch: pack corpora before the single synthesis call."""
    mode = int(row.get("autonomy_mode") or 0)
    retrieval = row.get("retrieval") if isinstance(row.get("retrieval"), dict) else None
    if mode != 0 or retrieval_mode(retrieval) != "deterministic_prefetch":
        return tools
    if any(str(tool.get("id") or "") == PREFETCH_SLOT_ID for tool in tools):
        return tools
    return [{"id": PREFETCH_SLOT_ID, "llm_role": "none", "invoke": {}}, *tools]
