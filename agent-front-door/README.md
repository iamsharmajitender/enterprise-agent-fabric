...
- [`05-reference/assistant-turn-request.json`](../docs/05-reference/assistant-turn-request.json)
- [`05-reference/assistant-turn-accepted.json`](../docs/05-reference/assistant-turn-accepted.json)
...
Jobs mint a new `idempotency_key` and payload ids. `correlation_id` comes back from Front Door. `claims_adjudicate` needs claim `claims:read`. Chats mint a new `session_id`; `fee_explain` needs `accounts:read`.
