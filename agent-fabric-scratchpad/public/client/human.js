...
import { addJobEvent, hideTyping, showTyping, } from "./thread.js";
export async function mountHuman(root) {
    let busy = false;
    ...
    const loadCatalog = async () => {
        ...
        if (!res.ok)
            throw new Error(`${res.status} loading /human.json`);
    ...
    const pollStatus = async (id) => {
        ...
        for (let i = 0; i < 120; i += 1) {
            ...
            if (ev.status === "completed" || ev.status === "failed") {
                ...
                addJobEvent(root, ev.status === "completed" ? "success" : "error", ev.status === "completed" ? "Result" : "Failed", seen.size ? "" : JSON.stringify(ev));
                return;
            }
    ...
    const submitResume = async () => {
        ...
        setBusy(true);
        try {
            const body = await client.fetchJson(`/v1/jobs/${encodeURIComponent(correlationId)}/turns`, { method: "POST", body: JSON.stringify(packet) });
    ...
    try {
        ...
        addJobEvent(root, "error", "Error", `Catalog unavailable (${err instanceof Error ? err.message : err})`);
    }
}
export function humanElements() {
    return {
        thread: document.getElementById("thread"),
        empty: document.getElementById("empty"),
        correlationInput: document.getElementById("correlation-id"),
        packetInput: document.getElementById("packet"),
