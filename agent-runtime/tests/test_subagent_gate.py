...
def test_merge_subagent_packet_into_stage_slot() -> None:
    ...
    assert merged["start_order"]["status"] == "completed"
    assert merged["start_order"]["result"] == {"flags": ["damage"]}
    assert merged["start_order"]["payload"] == {"order_id": "ORD-1"}
...
def test_agent_kind_join_raises_subagent_waiting() -> None:
        ...
        [
            {
                ...
                "invoke": {
                    "body": {"route_id": "billing_duplicate"},
                    "join": True,
                },
            }
        ],
    ...
    try:
        ...
        assert exc.stage_id == "start_billing"
        assert exc.state["slots"]["start_billing"]["status"] == "running"
        return
